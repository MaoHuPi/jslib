...
GDiv = document.createElement('ol'), 
CopyInnerText = document.createElement('p'), 
CopyInnerTextA = document.createElement('a');
// 
function setTDH(f){
    TDH.forEach(f);
}
...
function FDiv(){
    ...
    fd.style.backgroundImage = 'url("https://maohupi.riarock.com/web/tool/318station/file/fullscreen.svg")';
    fd.style.backgroundSize = '100%';
    fd.style.backgroundRepeat = 'no-repeat';
    fd.style.margin = '0px';
    fd.i_html = '';
    fd.addEventListener('click', MDiv_open);
    return(fd);
}
// 
function CDiv(){
    ...
    cd.style.backgroundImage = 'url("https://maohupi.riarock.com/web/tool/318station/file/content_copy.svg")';
    cd.style.backgroundSize = '75%';
    cd.style.backgroundRepeat = 'no-repeat';
    cd.style.backgroundPosition = 'center';
    cd.style.margin = '0px';
    cd.i_html = '';
    cd.addEventListener('click', HWT_copy);
    return(cd);
}
// 
MDiv.style.backgroundColor = '#00000088';
MDiv.style.position = 'fixed';
MDiv.style.width = '100vw';
MDiv.style.height = '100vh';
MDiv.style.left = '0px';
MDiv.style.top = '0px';
MDiv.style.zIndex = '9999';
MDiv.style.opacity = '0';
MDiv.style.pointerEvents = 'none';
MDiv.style.transition = '0.5s';
MDiv.style.border = 'none';
MDiv.style.boxSizing = 'border-box';
MDiv.style.margin = '0px';
// 
BDiv.style.backgroundColor = '#ffffff';
BDiv.style.position = 'absolute';
BDiv.style.width = '95vw';
BDiv.style.height = 'calc(100vh - 5vw)';
BDiv.style.left = '2.5vw';
BDiv.style.top = '2.5vw';
BDiv.style.zIndex = '10000';
BDiv.style.borderRadius = '1vw';
BDiv.style.border = 'none';
BDiv.style.boxSizing = 'border-box';
BDiv.style.margin = '0px';
// 
GDiv.style.border = 'none';
GDiv.style.boxSizing = 'border-box';
GDiv.style.margin = '0px 0px 0px 1em';
GDiv.setAttribute('id', 's1_GDiv');
// 
CopyInnerText.style.width = '0px';
CopyInnerText.style.height = '0px';
CopyInnerText.style.zIndex = '1';
CopyInnerText.style.opacity = '0';
CopyInnerText.style.pointerEvents = 'none';
CopyInnerText.style.fontSize = '0px';
CopyInnerText.style.userSelect = 'text';
CopyInnerText.setAttribute('id', 's1_CopyInnerText');
// 
MDiv.appendChild(BDiv)
BDiv.appendChild(GDiv)
CopyInnerText.appendChild(CopyInnerTextA);
body.appendChild(CopyInnerText);
function MDiv_open(){
    GDiv.innerHTML = this.i_html;
    whileGDI();
...
function HWT_copy(){
    userCopy('text', this.i_html);
}
MDiv.addEventListener('click', MDiv_close);
BDiv.addEventListener('click', (e) => {
    e.stopPropagation();
});
window.addEventListener('keydown', (e) => {
    if(e.key == 'Escape'){MDiv_close();}
    else if(e.key == 'f'){
        if(MDiv.style.pointerEvents == 'none'){MDiv_open();}
        else{MDiv_close();}
    }
});
